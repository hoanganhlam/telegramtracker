<template>
  <header class="sticky top-0 border-b py-2 bg-gray-800 text-white -mx-4 px-4 z-10 font-bold">
    <div class="mx-auto flex gap-4 items-center justify-between text-xs">
      <div class="flex-1 flex items-center gap-6">
        ...
        <client-only>
          ...
          <template v-if="value">
            <div class="hidden md:flex items-center gap-3 fill-gray-50 text-gray-50">
              <icon name="chevron-right"/>
              <nuxt-link class="text-sm capitalize" :to="`/${$route.params.type}`">{{$route.params.type}}</nuxt-link>
            </div>
            <div class="flex-1 hidden md:flex items-center gap-3 fill-gray-50 text-gray-50">
              <icon name="chevron-right"/>
